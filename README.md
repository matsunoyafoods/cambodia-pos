...
9. [x] [マルチテナントSaaS化 Phase A] `supabase/migrations/0002_pos_native_multitenant.sql` — 2026-08-29 適用完了。POS 単体運用用の `pos.stores`/`pos.staff`/`pos.menu_*`/`pos.integrations` を追加 (matsunoya-dine への依存なしで POS 単体を動かせるようにする第一歩)。松之屋フーズの現行運用への影響なし。詳細は `docs/multi-tenant-productization-spec.md`
