  ...
  return (
    <div className="flex h-dvh w-full flex-col overflow-hidden bg-background">
      <div className="flex flex-shrink-0 items-center gap-3 border-b border-border px-5 py-3">
        ...
        <div className="text-[15px] font-bold">{t('insights.title')}</div>
      </div>
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="rounded-xl border border-border bg-card p-5">
        <div className="mb-3 text-[13.5px] font-semibold">{t('insights.periodLabel')}</div>
        <div className="flex flex-wrap items-center gap-2.5">
          ...
          <button onClick={run} disabled={loading} className="h-10 rounded-lg bg-primary px-5 text-[13px] font-bold text-primary-foreground disabled:opacity-50">
            {loading ? t('insights.analyzing') : t('insights.runButton')}
          </button>
        </div>
        <p className="mt-2.5 text-[11.5px] text-muted-foreground">{t('insights.periodNote')}</p>
      </div>

      {error && <div className="rounded-lg border border-destructive/40 bg-destructive/5 p-4 text-[13px] text-destructive">{error}</div>}

      {loading && <div className="text-[13px] text-muted-foreground">{t('insights.analyzing')}</div>}
      ...
      {result && !loading && (
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="mb-2 text-[13.5px] font-semibold">{t('insights.findingsLabel')}</div>
          ...
          <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-5">
            <div className="mb-2 text-[13.5px] font-semibold text-emerald-800">{t('insights.suggestionsLabel')}</div>
            <ul className="flex flex-col gap-2">
          ...
          <div className="rounded-xl border border-border bg-secondary/20 p-5 text-[12px] text-muted-foreground">
            <div className="mb-1 font-semibold text-foreground">{t('insights.sourceDataLabel', { from: result.current.from, to: result.current.to })}</div>
            <div>
