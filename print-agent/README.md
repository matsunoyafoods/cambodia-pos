...
下記「0. 店舗にPCが無い場合: PassPRNT方式」(Bluetoothプリンター) か
「0b. 店舗にPCが無い場合: WebUSB方式」(USBプリンター・Android端末限定) を読んでください。**
...
## 0b. 店舗にPCが無く、USBプリンターをレジ端末(Android)に直接挿す場合: WebUSB方式 (中継機不要) (2026-09-21 追加)

レジ端末が **Android タブレット/スマホ + Chrome** の場合、USB接続のレシートプリンターを
その端末に直接挿すだけで、中継PC・専用アプリ無しにブラウザから直接印刷できます (WebUSB API)。
この `print-agent` (agent.mjs) は使いません。

**iPad/iPhone (iOS Safari) では使えません** (iOSはWebUSB自体に非対応)。iOSでUSBプリンターを
使いたい場合は、上記「0. PassPRNT方式」(プリンターがBluetooth対応機種の場合) か、下記
「1. 中継PC方式」(USB接続のまま中継Mac/PCを別途用意する) のどちらかになります。

1. プリンターをレジ端末(Android)にUSBケーブルで直接接続する (OTG変換ケーブルが必要な場合が
   あります)。
2. POSの設定画面 → プリンター設定 → プリンター追加で、接続方法に「USB接続 (レジ端末のブラウザ
   から直接印刷、Android Chromeのみ・中継機不要)」を選択する。
3. 画面に表示される「USBペアリング」ボタンを押すと、ブラウザ標準のデバイス選択ダイアログが
   出るので、一覧からプリンターを選んで「接続」する (この操作は一度だけでよい。ブラウザが
   このサイトへの許可を記憶する)。
4. 「テスト印刷」ボタンで印字を確認する。

うまくいかない場合、Androidの設定でこのプリンターが既に「USBプリンター」として認識・専有
されていると、ブラウザ側から掴めないことがあります。その場合はAndroid側のプリンター設定で
一旦無効化してから試してください。
